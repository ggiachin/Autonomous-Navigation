...
class DynamicNavigation:
    ...
    def init_app(self):
        ...
        rospy.Subscriber("/camera/rgb/image_raw", RosImage, self.__image_cbk)
        # Publishers
        self.path_pub = rospy.Publisher('global_plan', Path, queue_size=1)
        self.cmd_vel_pub = rospy.Publisher('cmd_vel', Twist, queue_size=1)
        self.image_pub = rospy.Publisher('mask_image',RosImage , queue_size=1)
    ...
    def __goal_pose_cbk(self, data):
        ...
        """
        self.goal_pose = data
        if self.goal_pose == self.ttbot_pose:
            self.endset = True
        else:
            self.endset = True
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            mask = getMask(self.image)
            rosImage_msg = self.bridge.cv2_to_imgmsg(mask, encoding="passthrough")
            self.image_pub.publish(rosImage_msg)
            trash_can_pix, num_pix = num_pixels(mask)
            alert_trash_can = trash_can_pix / num_pix * 100000
            ...
            if idx > len(path.poses) - 1:
                ...
                # Wait till trash can is no longer in front of robot to proceed with path
                if alert_trash_can > 60:
                    speed = 0
                    heading = 0
...
if __name__ == "__main__":
    nav = DynamicNavigation(node_name='DynamicNavigation')
    nav.init_app()
    try:
        nav.run()
